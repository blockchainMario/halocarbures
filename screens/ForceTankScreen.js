...
class ForceTankScreen extends Component {
  ...
  savetankfull = (navigation, t) => {
    var valid = true;
    if (valid) {
    //alert("http://18.190.29.217:8080/savefulltank/"+GLOBALS.UUID+"/"+this.state.fullDate+"/"+this.state.tank.partialQty);
    axios.get("http://18.190.29.217:8080/savefulltank/"+GLOBALS.UUID+"/"+this.state.fullDate+"/"+this.state.tank.partialQty
    , {
      headers: {
        'Accept': 'application/json',
        'Authorization': 'Bearer '+GLOBALS.BEARERTOKEN
