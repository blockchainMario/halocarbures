...
class TankDisposalScreen extends Component {
  ...
  savetankdisposal = (navigation) => {
    //alert("http://18.190.29.217:8081/savetankdisposal/"+GLOBALS.UUID+"/"+this.state.disposalDate+"/"+this.state.ticketId
    //+"/"+this.state.disposalEmployee+"/"+this.state.provider);
    axios.get("http://18.190.29.217:8081/savetankdisposal/"+GLOBALS.UUID+"/"+this.state.disposalDate+"/"+this.state.ticketId
    +"/"+this.state.disposalEmployee+"/"+this.state.provider
    , {
      headers: {
